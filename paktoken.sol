...
contract paktoken is Ownable, ERC721 {
    ...
    */
    ...
    // set price the token.
    function setSalesPrice(uint price) public onlyOwner {
        salePrice = price;
    } 
   ...
   // }
    ...
    function StartSales(bool status) external onlyOwner {
         if(isSaleStarted==true)                 // once started can not reset.
         return;
         else
         {
             isSaleStarted=status;
             timelocked = block.timestamp + 30 days;                       // once started , end within 30 days.
         }
    ...
    function buyNft(uint quantity) external payable {
        ...
        require(isSaleStarted==true,"Sales not started.");
        require(block.timestamp < timelocked, "Sales time finished."); 
